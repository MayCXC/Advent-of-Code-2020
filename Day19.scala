...
object Day19 {
    ...
    def otherzero(outer: String): Boolean = {
        ...
        def palindrome(inner: String): Boolean = inner.length <= 1 || (
            inner.indices.drop(1).map(inner.splitAt).unzip match { case (p, s) =>
                for(prefix <- p.filter(fortytwo.matches); suffix <- s.filter(thirtyone.matches))
                    yield(prefix, suffix)
            })
            .exists{ case (p, s) =>
                p.length + s.length <= inner.length && 
